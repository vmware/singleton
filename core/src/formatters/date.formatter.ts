...
export class DateFormatter {
    ...
    */
     ...
     */
    public getformattedString(date: Date, pattern: string, localeData: any, minusSign: string = '-', timezone?: string): string {
    ...
    public getRulesByPattern(pattern: string, localeData: any) {
        this.localeData = localeData;
    ...
    private formatDateTimeRules( type: string, rulesArray: string[]) {
        ...
        const rules = format.replace(/\{([^}]+)}/g, function(match: any[], key: number) {
    ...
    private patternFilter(pattern: string): any[]  {
        ...
        const mainWord = 'GyMLwWdEahHKmsSzZO',
            wordRange =
            'G{1,5}|y{1,4}|M{1,5}|L{1,5}|w{1,2}|W{1}|d{1,2}|E{1,6}|a{1,5}|h{1,2}|H{1,2}|K{1,2}|m{1,2}|s{1,2}|S{1,3}|z{1,4}|Z{1,5}|O{1,4}';
