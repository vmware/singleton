    ...
    def self.single_load(component, locale)
      ...
      @@load_threads_lock.synchronize do
        ...
        if thread.nil? || thread.alive? == false
          SgtnClient.logger.info { sprintf("%-18s %-16s %-15s %s", Thread.current.name, "[#{__method__}]", "[#{component}.#{locale}]", "Starting a new thread") }
          thread = Thread.new(Thread.current.name) do |parent_thread_name|
            Thread.current.name = (0...3).map { (65 + rand(26)).chr }.join
            ...
            SgtnClient.logger.info { sprintf("%-18s %-16s %-15s %s", Thread.current.name, "[#{__method__}]", "[#{component}.#{locale}]", "Thread is going to end") }
            bundle
          end
          @@load_threads[cache_key] = thread
        else
          SgtnClient.logger.info { sprintf("%-18s %-16s %-15s %s", Thread.current.name, "[#{__method__}]", "[#{component}.#{locale}]", "Another thread is alive") }
        end
        return thread
      end
    end
      ...
      @@refresh_threads_lock.synchronize do
        ...
        if thread.nil? || thread.alive? == false
          ...
          if expired || items.nil?
            SgtnClient.logger.info {message = expired ? "Cache expired" : "Initialize cache"
               sprintf("%-18s %-16s %-15s %s", Thread.current.name, "[#{__method__}]", "[#{component}.#{locale}]", "Starting a new thread: #{message}") }
            thread = Thread.new(Thread.current.name) do |parent_thread_name|
              Thread.current.name = (0...3).map { (65 + rand(26)).chr }.join
              Thread.current.name = "#{parent_thread_name}:#{Thread.current.name}"
              SgtnClient.logger.info { sprintf("%-18s %-16s %-15s %s", Thread.current.name, "[#{__method__}]", "[#{component}.#{locale}]", "Thread is started") }
            ...
            SgtnClient.logger.info { sprintf("%-18s %-16s %-15s %s", Thread.current.name, "[#{__method__}]", "[#{component}.#{locale}]", " Not expired") }
          end
        else
          SgtnClient.logger.info { sprintf("%-18s %-16s %-15s %s", Thread.current.name, "[#{__method__}]", "[#{component}.#{locale}]", "Another thread is alive") }
        end
