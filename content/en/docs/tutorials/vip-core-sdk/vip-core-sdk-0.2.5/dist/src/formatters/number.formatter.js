...
const number_format_util_1 = require("./number.format.util");
...
class Formatter {
    decimal(data) {
        const decimalFormats = data.numberFormats.decimalFormats, symbol = data.numberSymbols;
        let formatsInfo = this.parseFormats(decimalFormats);
        return (value, min, max) => {
            if (utils_1.isDefined(max) || utils_1.isDefined(min)) {
                formatsInfo = this.resetFormats(formatsInfo, min, max, number_format_util_1.RoundingMode.ROUND_HALF_EVEN);
            }
            return this.resetString(false, formatsInfo, symbol, value, max, min);
        };
    }
    currencies(data) {
        const currencyFormats = data.currencyFormats, symbol = data.numberSymbols;
        return (value, currencyCode, min, max) => {
    ...
    percent(data) {
        const percentFormats = data.numberFormats.percentFormats, symbol = data.numberSymbols;
    ...
    parseFormats(format, minusSign = '-') {
        const patternInfo = {
            ...
            'gSize': 0,
            'lgSize': 0,
            'round': number_format_util_1.RoundingMode.ROUND_HALF_EVEN
        };
        ...
        const integer = positiveParts[0], fraction = positiveParts[1] || '';
        ...
        if (negative) {
            const trunkLen = positive.length - patternInfo.posPre.length - patternInfo.posSuf.length, pos = negative.indexOf(DIGIT_CHAR);
    ...
    roundingNumber(number, minFrac, maxFrac, mode) {
        ...
        const newDecimal = new decimal_js_light_1.Decimal(number);
        const fractionSize = Math.min(Math.max(minFrac, fractionLen), maxFrac);
        const roundedNum = newDecimal.toFixed(fractionSize, mode);
        return roundedNum;
    }
    resetCurrencyFormatsInfo(formatsInfo, data, currencyCode, min, max) {
        if (!data.fractions[currencyCode]) {
            return formatsInfo;
        }
        const minFrac = data.fractions[currencyCode][number_format_util_1.CurrenciesDataType.DIGIST] || formatsInfo.minFrac;
        formatsInfo.minFrac = utils_1.isDefined(min) ? min : minFrac;
        formatsInfo.maxFrac = utils_1.isDefined(max) ? max : formatsInfo.minFrac;
        const rounding = data.fractions[currencyCode][number_format_util_1.CurrenciesDataType.ROUNDING];
        formatsInfo.round = !rounding || rounding === '0' ? formatsInfo.round : rounding;
        return formatsInfo;
    }
    resetPercentNumber(num) {
        return new decimal_js_light_1.Decimal(num).times(100).valueOf();
    }
    resetString(isCurrency, formatsInfo, symbol, value, minFracDigit, maxFracDigit) {
        let formattedText;
...
class FormatterFactory {
    ...
    currencies(data, locale) {
        let formatter = this.getFormatter(locale, number_format_util_1.NumberFormatTypes.CURRENCIES);
        if (!formatter) {
            formatter = this.formatter.currencies(data);
            this.mapping[locale][number_format_util_1.NumberFormatTypes.CURRENCIES] = formatter;
        }
        return formatter;
    }
    percent(data, locale) {
        let formatter = this.getFormatter(locale, number_format_util_1.NumberFormatTypes.PERCENT);
        if (!formatter) {
            formatter = this.formatter.percent(data);
            this.mapping[locale][number_format_util_1.NumberFormatTypes.PERCENT] = formatter;
        }
        return formatter;
    }
    decimal(data, locale) {
        let formatter = this.getFormatter(locale, number_format_util_1.NumberFormatTypes.DECIMAL);
        if (!formatter) {
            formatter = this.formatter.decimal(data);
            this.mapping[locale][number_format_util_1.NumberFormatTypes.DECIMAL] = formatter;
        }
        return formatter;
    }
    roundNumberForPlural(data, locale) {
        let formatter = this.getFormatter(locale, number_format_util_1.NumberFormatTypes.PLURAL);
        if (!formatter) {
            formatter = this.formatter.plural(data);
            this.mapping[locale][number_format_util_1.NumberFormatTypes.PLURAL] = formatter;
        }
        return formatter;
    }
}
