...
public class ProductService implements IProductService {
   ...
   @Override
   public List<String> getSupportedLocaleList(String productName, String version)
         throws L3APIException {
      List<String> supportedLocaleList = new ArrayList<String>();
      try {
         supportedLocaleList = productdao.getLocaleList(productName, version);
         if (supportedLocaleList != null) {
            supportedLocaleList.remove(ConstantsKeys.LATEST);
         }
      } catch (DataException e) {
         throw new L3APIException(ConstantsKeys.FATA_ERROR + "Failed to get locale list for "
               + productName + ConstantsChar.BACKSLASH + version, e);
      }
   ...
   public List<String> getComponentNameList(String productName, String version)
      ...
      try {
         ...
         throw new L3APIException(ConstantsKeys.FATA_ERROR + "Failed to get component list for "
               + productName + ConstantsChar.BACKSLASH + version, e);
      }
   ...
   public DropVersionDTO getVersionInfo(String productName, String version) throws L3APIException {
      ...
      try {
         ...
         throw new L3APIException(ConstantsKeys.FATA_ERROR + "Failed to drop version info for "
               + productName + ConstantsChar.BACKSLASH + version, e);
      }
      JSONObject jo = JSONUtils.string2JSON(jsonStr);
      String dropId =
            jo.get(ConstantsKeys.DROP_ID) == null ? "" : (String) jo.get(ConstantsKeys.DROP_ID);
         ...
         for (String componentName : s) {
            ...
            if (versionObj != null) {
               ...
               for (String locale : vset) {
                  ...
                  v.setLocale(locale);
                  v.setVersion(
                        versionJObj.get(locale) == null ? "" : (String) versionJObj.get(locale));
   ...
   public List<TranslationDTO> updateBatchTranslation(
         List<ComponentMessagesDTO> componentMessagesDTOList) throws L3APIException {
      ...
      for (ComponentMessagesDTO componentMessagesDTO : componentMessagesDTOList) {
         try {
            ...
            throw new L3APIException("Cache occurs error when update translation.", e);
         } catch (DataException e) {
            logger.error(e.getMessage(), e);
            throw new L3APIException(
                  "Failed to update translation for " + componentMessagesDTO.getProductName()
                        + ConstantsChar.BACKSLASH + componentMessagesDTO.getVersion(),
                  e);
         }
      }
   ...
   public boolean updateTranslation(ComponentMessagesDTO componentMessagesDTO)
      ...
      result = JSON.parseObject(
            TranslationCache3.getCachedObject(CacheName.ONECOMPONENT, key, String.class),
      ...
      if (StringUtils.isEmpty(result)) {// not exist in cache
         TranslationCache3.addCachedObject(CacheName.ONECOMPONENT, key, String.class,
               JSON.toJSONString(componentMessagesDTO));
         updateFlag = oneComponentDao.update(componentMessagesDTO.getProductName(),
               componentMessagesDTO.getVersion(), componentMessagesDTO.getComponent(),
         ...
         TranslationCache3.updateCachedObject(CacheName.ONECOMPONENT, key,
               String.class,  JSON.toJSONString(componentMessagesDTO));
      }
      return updateFlag;
   }
    ...
    * @param componentMessagesDTO
    *           the object of ComponentMessagesDTO, containing the latest translation.
    * @return ComponentMessagesDTO a DTO object of ComponentMessagesDTO, containing the all
    *         translation.
    * @throws ParseException
    * @throws DataException
    */
      ...
      if (result != null) {
         ...
         if (!StringUtils.isEmpty(messageObj)) {
            ...
            Iterator<Map.Entry<String, Object>> it =
                  ((Map) componentMessagesDTO.getMessages()).entrySet().iterator();
            while (it.hasNext()) {
    ...
    */
   private ComponentMessagesDTO getLinkedTranslation(ComponentMessagesDTO componentMessagesDTO)
         throws DataException {
      SingleComponentDTO caseComponentMessagesDTO = new SingleComponentDTO();
      String result = oneComponentDao.get2JsonStr(componentMessagesDTO.getProductName(),
            componentMessagesDTO.getVersion(), componentMessagesDTO.getComponent(),
            componentMessagesDTO.getLocale());
   ...
   public List<String> getSupportedLanguageList(String productName, String version)
         throws L3APIException {
