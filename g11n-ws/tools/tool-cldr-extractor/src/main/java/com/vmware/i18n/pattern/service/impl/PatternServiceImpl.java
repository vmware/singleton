...
public class PatternServiceImpl implements IPatternService {
	...
	static {
		...
		if (CLDRConstants.JSON_PATH.lastIndexOf(".jar") > 0) {
			...
			regionResult = LocalJSONReader.readJarJsonFile(CLDRConstants.JSON_PATH, CLDRConstants.REGION_LANGUAGES_PATH);
	 ...
	 *            locale. e.g. de, fr.
	 * @param categories
	 *            The pattern categories: dates, numbers, plurals, measurements, currencies.
	 *            use ',' to split. e.g. "dates,numbers,plurals", if null, will
	 *            return all category pattern.
	 * @return pattern
	 */
