 ...
 */
package com.vmware.vip.fetcher.common.resourcefile;
...
public class ResourceFilePathGetter {
    ...
    public static String getLocalizedJSONFileName(String locale) {
        if (StringUtils.isEmpty(locale) || locale.equals(ConstantsUnicode.EN)) {
            return ConstantsFile.LOCAL_FILE_SUFFIX + ConstantsChar.UNDERLINE
                    + ConstantsUnicode.EN + ConstantsFile.FILE_TPYE_JSON;
        } else {
    ...
    public static String getLocalizedJSONFileName(String filePrefix, String locale) {
        if (StringUtils.isEmpty(locale) || locale.equals(ConstantsUnicode.EN)) {
            return filePrefix + ConstantsChar.UNDERLINE + ConstantsUnicode.EN
                    + ConstantsFile.FILE_TPYE_JSON;
        } else {
            return filePrefix + ConstantsChar.UNDERLINE + locale + ConstantsFile.FILE_TPYE_JSON;
        }
    }
    ...
    public static String getLocalizedPropertiesFileName(String filePrefix, String locale) {
        if (locale.equals(ConstantsUnicode.EN_US) || locale.equals(ConstantsUnicode.EN)) {
            return filePrefix + ConstantsFile.FILE_TPYE_PROPERTIES;
        } else {
            return filePrefix + ConstantsChar.UNDERLINE + locale
                    + ConstantsFile.FILE_TPYE_PROPERTIES;
        }
    }
    ...
    public static String getLocalizedJSONFilesDir(String generatedFilePath,
        ...
        String jsonFilePathDir = generatedFilePath + ConstantsChar.BACKSLASH
                + ResourceFilePathGetter.getProductVersionConcatName(singleComponentDTO)
                + ConstantsChar.BACKSLASH + singleComponentDTO.getComponent();
        System.out.println("json file path: " + jsonFilePathDir);
