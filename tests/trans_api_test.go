...
func TestGetBundleNormal(t *testing.T) {
	...
	params := e.GET(GetBundleURL, Name, Version, "zh-Hans", "sunglow")
	resp := params.Expect()
	...
	params = e.GET(GetBundleURL, Name, Version, "zh-Hans", "sunglow")
	resp = params.WithHeader(headers.IfNoneMatch, etag).Expect()
...
func TestPutBundleWithoutData(t *testing.T) {
	...
	for _, tt := range tests {
		...
		t.Run(tt.testName, func(t *testing.T) {
			...
			params := translation.UpdateTranslationDTO{}
			json.Unmarshal([]byte(fmt.Sprintf(bundleDataToPut, tt.name, tt.version, tt.locale, component, Key, Msg)), &params)
			params.Data.Translation = nil
