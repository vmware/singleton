...
describe "tmplete 1 test" do
    ...
    context "getString_f translation" do
        ...
        it "Get a string's translation and componet in bundle" do
            expect(Sgtn.translate('contact.welcome3', "contact", 'en-US')).to eq("%{name}, welcome contact login %{place}!")
        end

        it "Get a string's translation and component in bundle and locale is de____new dleng bug" do
            expect(Sgtn.translate('contact.welcome3', "contact", 'de')).to eq("%{name}, welcome contact login de online %{place}!")
        end
        ...
        #     puts(Sgtn.get_translations("about", 'en')["messages"])
        # end

        it "Get a string's translation and component in source" do
            expect(Sgtn.translate("about2.key2", "onlyonline", 'de')).to eq("test value de 2")
        end

        it "Get a string's translation and the component only in offline" do
            expect(Sgtn.translate('onlyoffline.key2', "onlyoffline", 'de')).to eq("test value de2")
        end
