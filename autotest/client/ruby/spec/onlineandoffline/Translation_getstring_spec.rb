...
describe "Translation test" do
    before :each do
        ...
        Sgtn.load_config("./config/sgtnclient.yml", "onlineandofflinemode")
    end
    ...
    context "Get string's translation" do
        ...
        it "Get a string's translation and component only in online" do
            expect(SgtnClient::Translation.getString("onlyonline", "about2.key2", "de")).to eq("test value de 2")
        end

        it "Get a string's translation and component only in offline" do
            expect(SgtnClient::Translation.getString("onlyoffline", "onlyoffline.key1", "de")).to eq("test value de1")
        end
        ...
        it "Get a string's translation and key not exist" do
            expect(SgtnClient::Translation.getString("about", "about.key134", "de")).to eq(nil)
        end

        it "Get a string's translation and key only in offline" do
            expect(SgtnClient::Translation.getString("contact", "contact.offline", "en")).to eq(nil)
        end
