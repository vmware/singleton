...
public class StringBasedOpt extends BaseOpt implements Opt {
	...
	public JSONObject getComponentMessages() {
		String responseStr = VIPCfg.getInstance().getVipService().getHttpRequester().request(V2URL
				.getComponentTranslationURL(dto, VIPCfg.getInstance().getVipService().getHttpRequester().getBaseURL()), ConstantsKeys.GET, null);
	...
	public String postString() {
		...
		String responseStr = VIPCfg.getInstance().getVipService().getHttpRequester().request(V2URL
				.getKeyTranslationURL(dto, VIPCfg.getInstance().getVipService().getHttpRequester().getBaseURL()), ConstantsKeys.POST, params);
	...
	public String postSourceSet(String sourceSet) {
		...
		String responseStr = VIPCfg.getInstance().getVipService().getHttpRequester().request(V2URL.getPostKeys(dto, VIPCfg.getInstance().getVipService().getHttpRequester().getBaseURL()), ConstantsKeys.POST, sourceSet);
	...
	public String getTranslationStatus() {
	    ...
	    String getURL = V2URL.getComponentTranslationURL(dto, VIPCfg.getInstance().getVipService().getHttpRequester().getBaseURL());
		String responseStr = VIPCfg.getInstance().getVipService().getHttpRequester().request(getURL, ConstantsKeys.GET, params);
		if (null == responseStr || responseStr.equals("")) {
			return status;
		} else {
