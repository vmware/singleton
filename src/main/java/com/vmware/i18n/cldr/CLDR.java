...
public class CLDR {
	...
	private String[] coreSubtags(String locale) {
		...
		if (aux.length > 1 && !CommonUtil.isEmpty(aux[1])) {
			...
			locale = aux[0].toString() + (subAux.length > 2 && CommonUtil.isEmpty(subAux[1]) ? "-t-" + subAux[1] : "");
		...
		// 0: language 1: script 2: territory (aka region) 3: variant 4:
	 ...
	 * unicode_variant_subtag)*; Although unicode_language_subtag = alpha{2,8}, I'm
	 * using alpha{2,3}. Because, there's no language on CLDR lengthier than 3
	 * 
